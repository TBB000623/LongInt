...
complex complex_expotional::operator()(int k, int n = 0) const {
	if (n == 0) {
		...
		return complex::complex_exp(kk, n);
	}
}
...
// LInt
LInt::LInt(bool b, int code = 0) : sign(0), d(0), num(1, 0) {
	if (b == false) {
		if (code > 0) sign = 2;
		if (code < 0) sign = -2;
		num.clear();
...
LInt::LInt(const char* in_string) {
	int i, len;
...
LInt::LInt(const u32* in_num, int k) : sign(1), d(k), num(in_num, in_num + k) { this->sho(); }
template <typename It>
LInt::LInt(It begin_iter, It end_iter) : sign(1), d(std::distance(begin_iter, end_iter)), num(begin_iter, end_iter) {
	this->sho();
}
#if __cplusplus >= 201103L
LInt::LInt(const string& inString_) : LInt(inString_.c_str()) {}
#else
...
// unary operaion
LInt LInt::operator<<(int k) const {
...
LInt LInt::operator*(int B) const {
	const LInt& A = *this;
...
// stream IO function
std::ostream& operator<<(std::ostream& os, const LInt& A) {
	if (A.sign == 0) {

...
LInt _recip_2(const LInt& A) {
	...
	u64 delta;
	if (_rA < 0)
		for (delta = 1; _rA.sign < 0; delta *= 2) {
...
LInt lcd(const LInt& a, const LInt& b) { return (a * b) / gcd(a, b); }
...
// Declare
template <>
LFloat mul_pow10(const LFloat&, int);
template <>
LFloat pow10(int);
LFloat agm_pi(void);
...
LFloat mod(const LFloat& x, const LFloat& y) { return x - trunc(x / y) * y; }
}  // namespace tbb
#endif  // TBBLFLT_H
